...
let lesinitiales = "INITIALS LIFELINE";
let blank = `<u>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</u>`;
...
function startGame() {
	...
	refreshGUI();
	for(i = 0; i < maxRounds; i++){
		roundBox[i].innerText = `${stops[i].words } WORDS`//'&#9642;&#65039;&#9642;&#65039;&#9642;&#65039;&#9642;&#65039;&#9642;&#65039;';
...
function nextRound(button) {
	if(window.getComputedStyle(button).getPropertyValue('opacity') == 1){
		player.currentTime = stops[round].time + timeAdjust - 5;
		round = round + 1;
		if(round == maxRounds){
			endGame();
			return;
...
let summaryAnswers = new Array(5).fill("[no response]");
...
function share(btn) {
	...
	ret += `Lyricdle:\n${ title }\n${ artiste }\n`;
	for(i=0; i < 5; i++){
		ret += `${ emojiCorrectness[i] } Round ${ i+1 }: `
		if(stops[i].words <= 6){
			ret += stops[i].teaser.toLocaleLowerCase() + ' ';
		}
		ret += `${ summaryEmoji[i].join('') } \n`;
	}
	ret += `https://guissmo.com/lyricdle.html\n`;
...
function showSummary() {
	...
	ret += `<center>`;
	ret += `<button class="button-below big-button share-button" onclick="share(this)">SHARE</button>`;
	ret += `</center>`;
	ret += `<br>`;
	ret += `<h3>Summary</h3>`;
	for(i=0; i < 5; i++){
...
function updateInputPreview() {
	...
	if(initialsUsed){
		for(i = 0; i < realAnswer.length; i++){
			if(realAnswer[i].length == 2 && realAnswer[i].charAt(0) == "'"){
				newArray[i] = realAnswer[i];
				console.log(2)
			} else if(realAnswer[i].length == 1){
                newArray[i] = realAnswer[i];
